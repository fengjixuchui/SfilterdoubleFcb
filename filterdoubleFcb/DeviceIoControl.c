						  ...
						  --*/
{
	...
	case FILESPY_StartLoggingDevice: //这个参数是驱动名，比如文件系统设备名什么的
	...
	case PFPCOMMAND_ModifyPSW:
		{
			...
			*(ULONG*)OutputBuffer	= 0;
			...
			{//just modify user psw
				...
				MD5Init(&ctx);
				pMd5Buffer	 = ExAllocatePool(PagedPool,sizeof(WCHAR)*(wcslen(pModify->szUserName)+wcslen(pModify->szOldUserPSW)+1));
				if(pMd5Buffer	 == NULL)
				{
					IoStatus->Status = STATUS_INVALID_PARAMETER;
					goto exit11;
				...
				pMd5Buffer = ExAllocatePool(PagedPool,sizeof(WCHAR)*(wcslen(pModify->szUserName)+wcslen(pModify->szNewUserPSW)+1));
				if(pMd5Buffer == NULL)
				{
					IoStatus->Status = STATUS_INVALID_PARAMETER;
					goto exit11;
			...
			}else if(pModify  ->nModifyType==2)
			{
				...
				MD5Init(&ctx);
				pMd5Buffer	 = ExAllocatePool(PagedPool,sizeof(WCHAR)*(wcslen(pModify->szOldKeyPSW)+1));
				if(pMd5Buffer	 == NULL)
				{
					IoStatus->Status = STATUS_INVALID_PARAMETER;
					goto exit11;
	...
	case PFPCOMMAND_DOLOGON:
		{
			...
			bHasLogon  = (g_pKeyContent!= NULL);
                        ...
                        //传递过来的文件路径
			pKeyFilePath = ExAllocatePool(PagedPool,sizeof(WCHAR)*(wcslen(pBuffer)+1));
			...
			if( sizeof(WCHAR)*(pBuffer-pHeadBuffer)>InputBufferLength)
			{
			    ...
			    goto exit;
                        ...
                        //传递过来的 KeyFile的密码
			pKeyPsw = ExAllocatePool(PagedPool,sizeof(WCHAR)*(wcslen(pBuffer)+1));
			if(pKeyPsw== NULL)
			{   
                            *(ULONG*)OutputBuffer = 3;
			    IoStatus->Status = STATUS_INSUFFICIENT_RESOURCES;
			...
			{
			    g_pKeyFileContent = ExAllocatePool(PagedPool,16);
			    if(g_pKeyFileContent== NULL )
			    {
                                 *(ULONG*)OutputBuffer = 3;   
			        IoStatus->Status = STATUS_INSUFFICIENT_RESOURCES;
			...
			MD5Init(&ctx);
			pMd5Buffer	 = ExAllocatePool(PagedPool,sizeof(WCHAR)*(wcslen(pUser)+wcslen(pPsw)+1));
			if(pMd5Buffer	 == NULL)
			{
                            *(ULONG*)OutputBuffer = 2;   
			    IoStatus->Status = STATUS_INVALID_PARAMETER;
...

			{
			    ...
			    pSample = ExAllocatePool(PagedPool,240);
			    if(pSample== NULL)
			    {
                                *(ULONG*)OutputBuffer = 3;   
			        IoStatus->Status = STATUS_INSUFFICIENT_RESOURCES;
			...
			*(ULONG*)OutputBuffer = 1;//success
			...
			if(pKeyFilePath)
			{
			    g_KeyFilePath = ExAllocatePool(PagedPool,sizeof(WCHAR)*(1+wcslen(pKeyFilePath)));
	...
	case PFPCOMMAND_GENKEYFILE:
		{
			...
			*(ULONG*)OutputBuffer = 0;
			...
			pQ1 = ExAllocatePool(PagedPool,sizeof(WCHAR)*(wcslen(pBuffer)+1));
			if(pQ1	 == NULL)
			{
				IoStatus->Status = STATUS_INSUFFICIENT_RESOURCES;
				goto exit;
			...
			pA1 = ExAllocatePool(PagedPool,sizeof(WCHAR)*(wcslen(pBuffer)+1));
			if(pA1	 == NULL)
			{
				IoStatus->Status = STATUS_INSUFFICIENT_RESOURCES;
				goto exit;
