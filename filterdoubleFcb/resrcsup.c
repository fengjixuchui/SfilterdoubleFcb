...

{
	...
	if (ExAcquireResourceShared( Fcb->Resource,(BOOLEAN) BooleanFlagOn(IrpContext->Flags, IRP_CONTEXT_FLAG_WAIT))) 
	{
...
{
	if(ExAcquireResourceExclusive(Fcb->Resource,BooleanFlagOn(IrpContext->Flags,IRP_CONTEXT_FLAG_WAIT)))
		return TRUE;
	else
...

{
	...
	{
		if(!ExAcquireResourceShared(Fcb->Header.Resource,FALSE))
		{
			return FALSE;	
		}else
		{
			ExReleaseResource(Fcb->Header.Resource);
			AcquiredFile = ExAcquireResourceShared( Fcb->Header.Resource, Wait );
		}
	} 
...

{
	...
	if ((Fcb->Header.Resource == NULL) ||
		ExAcquireResourceShared( Fcb->Header.Resource, Wait ))
	{
		AcquiredFile = TRUE;			 
	}
