	...
	case FILEOBJECT_FROM_USERMODE:
		{
			...
			except (PfpExceptionFilter ( Irp_Context, GetExceptionInformation()))
			{
...
{	
	...
	try
	{
			...
			(FileObject->SectionObjectPointer->DataSectionObject != NULL))
		{
				...
				ExAcquireResourceShared( pFcb->Header.Resource, TRUE );

				//if (Scb->CompressionUnit == 0) //这个注释掉了，可能双缓冲是不需要Scb的
				{
					...
					ExReleaseResource( pFcb->Header.Resource );
					ExAcquireResourceExclusive( pFcb->Header.Resource, TRUE );
					ExReleaseResource( pFcb->Header.Resource );
		...
		//if (!NonCachedIo)
		{
				...
				if (!PagingIo)
				{
					...
					if (!Wait && NonCachedIo) 
					{
						...
						if (ExIsResourceAcquiredShared(pFcb->Header.Resource) > 10) 
						{
							PfpRaiseStatus( IrpContext, STATUS_CANT_WAIT, NULL );
						}
...
// 					}
				...
				} else if ((pFcb->Header.Resource != NULL) &&!ExIsResourceAcquiredShared(pFcb->Header.Resource)) 
				{
					//
					//  If we cannot acquire the resource, then raise.
					//
				
						if (!ExAcquireResourceShared( pFcb->Resource, Wait )) 
						{
							PfpRaiseStatus( IrpContext, STATUS_CANT_WAIT, NULL );
						}					
...
// 						}
		...
		//if (TypeOfOpen == UserFileOpen) 这个是自己的fcb，默认就是打开的。
		{
		...
		if (NonCachedIo) 
		{
			...
			BytesToRead = ((ULONG)ByteCount + (SectorSize - 1)) & ~(SectorSize - 1); //字节对齐
				...
				|| BytesToRead > IrpSp->Parameters.Read.Length)) 
			{
					...
					/*if (IrpContext->pasencrypt == TRUE)
					{
						NewStart = 0;
						NewEnd = ENCRYPTIONHEADLENGTH;
						RequestedByteCount = ENCRYPTIONHEADLENGTH;
					}*/
				   ...
				   )
{
	...
	pUserFileObejct	    = pIoPreStack->FileObject;//这个是用户掌握的文件对象，也是假的FCB
								...
								)
{
	...
	{
		//不需要加密的话就不解密（例如设定文件类型不加密）
		//这个是假的文件对象的FCB，晕，看得晕乎乎的
							   ...
							   )
{
	...
	if( pResource )
	{
		ExReleaseResourceForThread(pResource,ResourceThreadId);	
	}
