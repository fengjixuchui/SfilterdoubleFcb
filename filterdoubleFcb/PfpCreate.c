                ...
                __in PIRP			Irp)
{
  ...
  //  }
   ...
   ProcessInfo = PfpGetProcessInfoForCurProc();
	...
	//PIDDD = PsGetCurrentProcessId();

	///////////////////////////////////////////////////////////////////////////////////////////检测流文件
	if (PfpFileObjectHasOurFCB(pFileObject->RelatedFileObject))
	{
		if (!bHasFileExt)// 没有文件后缀或者没有设置过的类型的文件
		{
			goto PASSTHROUGH;// PASS 掉！ 不处理
		}
		if (bOpenFileStream)
	...
	///////////////////////
	  if(IsFileTypeBelongExeType(szExt)||!PfpIsThereValidProcessInfo()||(ProcessInfo= (IrpContext?IrpContext->pProcessInfo:PfpGetProcessInfoForCurProc()))== NULL)//没有找到这个对应的程序 ，或者 这个程序当前是禁止加密的
	  {		
	      //处理3种情况：对当前打开文件的请求
	      //1：是exe类型的文件！系统不会加密
	      //2: 当前的进程不是加密进程！
...
//DbgPrint("[Wrench]存在这个进程，进程名:%ws\r\n", ProcessInfo->ProcessName.Buffer);
///DbgBreakPoint();
FOLDERANDNOFOLDER: //下面的就是个人安全文件夹和可信进程访问的时候都要做的
    ...
    {
        bFirstOpen  = TRUE;
		//DbgPrint("[Wrench]bFirstOpen File PID:%d\r\n", PsGetCurrentProcessId());
    }
...
EXIT:
    ...
    if(pDeviceResouce)
    {
        ExReleaseResource(pDeviceResouce);
    }
    ...
    try
    {
        ...
        case ACCESSING_FILE_EXIST:
		{
			...
			else
			{
				...
				DbgPrint("[Wrench]创建了我们自己的Fcb Fcb=%08x\r\n", (*pDiskFileObject)->pFCB);
				...
				{
					ppFcbCreated->Encryptbyer = FALSE;
				}*/
				if (NT_SUCCESS(ioStatus.Status))
				{
...
PPROCESSINFO PfpGetProcessInfoForCurProc()
{
	...
	UCHAR str[513] = "";
    pszHashValue = ExAllocatePoolWithTag(NonPagedPool,PROCESSHASHVALULENGTH,'1102');
	imageName = ExAllocatePoolWithTag(NonPagedPool, PROCESSHASHVALULENGTH, '1103');


    if(pszHashValue== NULL&& imageName==NULL)
    {
        return NULL;
    }
...
//////////////////////////////////////////////////////////////////////////////
		...
		//pszHashValue = "1234567890";
		//////////////////////////////////////////////////////////////////////////////
        ProcessInfo = PfpGetProcessInfoUsingHashValue(str,PROCESSHASHVALULENGTH,NULL); //这里改了下
                            ...
                            IN OUT LONGLONG *		FileSize)
{
	...
	BOOLEAN           FileAccessedEncrypt = FALSE;
    BOOLEAN			bDir = FALSE;
    ...
    *bDir = FALSE;
    ...
    if((FileDirectoryEntry->FileAttributes&FILE_ATTRIBUTE_DIRECTORY) !=FILE_ATTRIBUTE_DIRECTORY )
    {
        *bReadonly = (FileDirectoryEntry->FileAttributes&FILE_ATTRIBUTE_READONLY)?TRUE:FALSE	;
		...
		if (*bReadonly==FALSE)
		{
			*bEncrypt = (FileDirectoryEntry->FileAttributes&FILE_ATTRIBUTE_ENCRYPTED) ? TRUE : FALSE;
		}
                               ...
                               FILESTATE			AcsType)
{
    ...
    ntstatus  = ZwCreateFile(	&pDiskFileObject->hFileWriteThrough,
        /*((AcsType== ACCESSING_FILE_EXIST_READONLY)?FILE_GENERIC_READ:(FILE_GENERIC_READ|FILE_GENERIC_WRITE))*/0,//*DesiredAccess| ((AcsType== ACCESSING_FILE_EXIST_READONLY)?0:FILE_WRITE_DATA)| SYNCHRONIZE  ,
        &Objattri,
        iostatus,									
        &AllocationFile,
        FileAttributes ,
        FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,//|FILE_SHARE_WRITE|FILE_SHARE_DELETE,
        CreateDisposition,
    ...
    */
    ...
    if(iostatus->Information == FILE_OPENED)
    {
        ...
        }else
        {
            ...
            {
                if(!PfpInitFCBFromEncryptBuffer(EncryptHead,ENCRYPTIONHEADLENGTH,pDiskFileObject->pFCB)) //这里，卧槽尼玛的
                {
