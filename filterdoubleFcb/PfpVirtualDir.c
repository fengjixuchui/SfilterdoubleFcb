...
PDISKDIROBEJECT PfpCreateVirtualDirObject(PWCHAR pDirName,PDISKDIROBEJECT pParent)
{
	...
	pDir = ExAllocateFromPagedLookasideList(&g_VirualDirLookasideList)	;
	...
	wcscpy(pDir->DirName.Buffer,pDirName);
...
PVIRTUALDISKFILE CreateVirDiskFileAndInsertIntoParentVirtual(PDISKDIROBEJECT pParent,PWCHAR szFileName)
{
	...
	pVirtualDiskFile = ExAllocateFromPagedLookasideList(&g_VirualDiskFileLookasideList);
	...
	InitializeListHead(&pVirtualDiskFile->listForDiskFileObject);//这里没上锁是怎么回事？
...
PfpCloseDiskFileObjectsUnderDir(PWCHAR pszFolderPath)
{
	...
	pPathWithoutDevice = ExAllocatePoolWithTag(PagedPool,wcslen((PWCHAR)pszFolderPath)*sizeof(WCHAR)/*sizefo*/,'1111');
	if(!pPathWithoutDevice )
	{
		goto DirectExit;
	}
	wcscpy(pPathWithoutDevice,&((PWCHAR)pszFolderPath)[2]);
