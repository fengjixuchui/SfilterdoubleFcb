...
BOOLEAN InitializeWChar(WCHAR** Global_Variable, WCHAR* Str) {
	...
	try
	{
		Global_Variable = ExAllocatePoolWithTag(NonPagedPool, (wcslen(Str) + 1) * sizeof(WCHAR), 'pfp0');
		if (Global_Variable == NULL)
		{
			return STATUS_INSUFFICIENT_RESOURCES;
		}
		RtlZeroMemory(DOCX, (wcslen(Str) + 1) * sizeof(WCHAR));
		wcscpy(DOCX, Str);
	}except(1){
		if (Global_Variable!=NULL)
		{
			ExFreePool(Global_Variable);
			return -1;
		}
	}
	return 0;
...
/*UNICODE_STRING pRegistryKeyPath*/)
{
	...
	{
		SpyStartLoggingDevice(Letter); //自动附加所有的盘符，汗，白忙活了
	}
    ...
    if (NULL != deviceObject) 
	{
		...
		if(PfpFileObjectHasOurFCB(FileObject))
		{
			...
			try
			{
				...
				//if (PfpGetProcessInfoForCurProc() == NULL)
				//{
				//	CcFlushCache(&((PPfpFCB)FileObject->FsContext)->SegmentObject, NULL, 0, &Iosb);
				//	//
				//	//  If no error then purge the section.
				//	//
				//	if (NT_SUCCESS(Iosb.Status))
				//	{
				//		CcPurgeCacheSection(&((PPfpFCB)FileObject->FsContext)->SegmentObject, NULL, 0, FALSE);
				//	}
				//}
			}
			finally
...
--*/
{
    ...
    if (NULL != deviceObject) 
	{
		...
		if(PfpFileObjectHasOurFCB(FileObject))
		{
			/*HANDLE  hProcess = PsGetProcessId(IoGetCurrentProcess());

			HANDLE  process_now = PsGetCurrentProcessId();


			PPROCESSINFO ProcessInfo = PfpGetProcessInfoUsingProcessId(hProcess);
			if (ProcessInfo == NULL)
			{
				goto PASSTHROUGH;
			}*/
			...
			//	{
			//		
			//		returnValue = (fastIoDispatch->FastIoWrite)(pDiskFileObject->pDiskFileObjectWriteThrough,
			//			FileOffset,
			//			Length,
			//			Wait,
			//			LockKey,
		...
		if(PfpFileObjectHasOurFCB(FileObject))
		{
			if (!PfpGetProcessInfoForCurProc())
			{
					...
					FastIoQueryBasicInfo))
				{
					 ...
					 returnValue = (fastIoDispatch->FastIoQueryBasicInfo)(pDiskFileObject->pDiskFileObjectWriteThrough,
						Wait,
						Buffer,
						IoStatus,
						deviceObject);
...
--*/
{
	...
	if(bComplete)
	{
		...
		if(pVirtualDiskFile )
		{
			...
			//	}
			//	ExReleaseResource(pVirtualRootDir->AccssLocker);
			//	FsRtlExitFileSystem();
			//	return FALSE;
			//}
			...
			if(pDiskFileObject)
			{
				...
				//{
				//	if (ties) {
				//		CcFlushCache(&((PPfpFCB)pDiskFileObject->pFCB)->SegmentObject, NULL, 0, &Iosb);
				//		//
				//		//  If no error then purge the section.
				//		//
				//		if (NT_SUCCESS(Iosb.Status))
				//		{
				//			CcPurgeCacheSection(&((PPfpFCB)pDiskFileObject->pFCB)->SegmentObject, NULL, 0, FALSE);
				//		}
