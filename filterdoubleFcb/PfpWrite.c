...
//   	ExAcquireResourceExclusiveLite(pDeviceResource,TRUE);
	...
	//{
	//	/*	pstack->FileObject = pDiskFileObject->pDiskFileObjectWriteThrough;
	//		pstack->FileObject->FsContext = pDiskFileObject->pDiskFileObjectWriteThrough->FsContext;
	//		goto PASSTHROUGH;*/
	//	Irp->IoStatus.Information = 0;
...

			{
				...
				else
				{
					...
					if(!pFcb->bWriteHead)
					{
						...
						if(pEncryptHead )
						{
							//这个是需要加密的文件，向下层ntfs发送写的加密，写到真实fcb的缓存上
