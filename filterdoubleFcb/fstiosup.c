...

{
	...
	BOOLEAN						 Status = TRUE;
	ULONG						 PageCount = COMPUTE_PAGES_SPANNED(((PVOID)(ULONGLONG)FileOffset->LowPart), Length);
	...
	if (Length != 0) 
	{
		...
		if ((Header->Resource == NULL) ||
			!ExAcquireResourceShared(Header->Resource, Wait))
		{
				Status = FALSE;
				goto Done2;
		}
		...
		if (BeyondLastByte.QuadPart > Header->ValidDataLength.QuadPart)
		{
			...
			DoingIoAtEof = !FlagOn( Header->Flags, FSRTL_FLAG_EOF_ADVANCE_ACTIVE ) ||
				PfpWaitForIoAtEof( Header, FileOffset, Length, &EofWaitBlock );
		...
		{	//
			...
			if (!PfpFastIoCheckIfPossible( FileObject,
				FileOffset,
				Length,
				Wait,
				LockKey,
		...
		(Header->Resource != NULL)) 
	{
			...
			if (Length != 0)
			{
				...
				} else 
				{
					...
					Status = ExAcquireResourceShared( Header->Resource, Wait );
					if (!Status)
					{
						goto Done2;
					}
...
Done1: ExReleaseResource( Header->Resource );
...

{
	...
	InsertTailList( Fcb->PendingEofAdvances, &EofWaitBlock->EofWaitLinks );
...

{
	...
	Fcb = (PPfpFCB)(Header);
	while(!IsListEmpty(Fcb->PendingEofAdvances))
	{
		EofWaitBlock = (PEOF_WAIT_BLOCK)RemoveHeadList( Fcb->PendingEofAdvances );
...
// 
// 		
// 		if(IsListEmpty(Fcb->PendingEofAdvances))
// 		{
